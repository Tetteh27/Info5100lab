...
public class DatabaseConnector {
    ...
    public static void addUser(User u1) throws SQLException{
        String query = "INSERT INTO student(first_name, last_name, age, gender, hobbies) VALUES (?, ?, ?, ?, ?) ";
        try{
            // Create a connection
            ...
            stmt.setString(5, u1.getHobbies());
    ...
    public static ArrayList<User> getUsers(){
        ...
        try{
            ...
            while(rs.next()){
                ...
                u1.setHobbies(rs.getString("hobbies"));
                users.add(u1);
            }
            rs.close();
            conn.close();
    ...
    public static void updateUser(User oldUser, User updatedUser){
            String query = "UPDATE student SET first_name = ?, last_name = ?, gender = ?, age = ?, hobbies = ? WHERE student_id = ? ";
            // Update the query
        try{
            Connection conn = DriverManager.getConnection(DB_URL, DB_USERNAME, DB_PASSWORD);
            PreparedStatement stmt = conn.prepareStatement(query);
            ...
            stmt.setString(5, updatedUser.getHobbies());
